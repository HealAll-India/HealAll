...
export default function OtpSignupPage() {
  ...
  function setRole(role: "helper" | "help_seeker", checked: boolean) {
    ...
    if (checked) roleSet.add(role); else roleSet.delete(role);
    setFormData(prev => ({ ...prev, roles: Array.from(roleSet) as SignupRequest["roles"] }));
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    setMessage(null);
    if (formData.roles.length === 0) { setError("Choose at least one role"); return; }
    setLoading(true);
    try {
  ...
  return (
    <main style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "calc(100vh - 60px)", padding: "2rem 1rem" }}>
      <div className="card stack" style={{ width: "100%", maxWidth: "440px", borderRadius: "20px", padding: "36px 32px" }}>
        ...
        <p className="muted" style={{ textAlign: "center", fontSize: "13px", marginBottom: "8px" }}>
          Sign up with email OTP
        </p>
        <p style={{ textAlign: "center", marginBottom: "16px" }}>
          <a href="/signup" style={{ fontSize: "12px", color: "#16a34a" }}>← Use Google instead (faster)</a>
        </p>
        ...
        <form className="grid" onSubmit={handleSubmit}>
          <label>Invite Code
            <input value={formData.invite_code} onChange={e => setFormData(prev => ({ ...prev, invite_code: e.target.value }))} placeholder="HEAL-XXXXXX" required />
          </label>
          <label>Full Name
            <input value={formData.name} onChange={e => setFormData(prev => ({ ...prev, name: e.target.value }))} placeholder="Your name" required />
          </label>
          <div className="row">
            <label style={{ flex: 1 }}>
              Phone
              <input value={formData.phone} onChange={e => setFormData(prev => ({ ...prev, phone: e.target.value }))} placeholder="9999999999" required />
